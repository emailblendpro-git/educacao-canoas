/**
 * Parser + validador de planilhas de organização escolar (formato "Quadro de
...
function parseEquipeDiretiva(ws) {
  ...
  ws.eachRow((row) => {
    ...
    if (!label) return;
    const key = normalize(label).replace(/[çc]/g, 'c');
...
async function gerarDevolutiva(pendencias, nomeEscola, outPath) {
...
async function importar(pool, filePath, { rosterValido, cargoPorMatricula, grade, alocacoesResolvidas, matriz, pendencias }) {
  const nomeEscola = nomeEscolaFromFilename(filePath);
  ...
  try {
    ...
    console.log(`\n[import] OK — escola "${nomeEscola}" (id=${escolaId})${escolaJaExistia ? ' [já existia, dados adicionados]' : ' [nova]'}:`);
    console.log(`  professores novos: ${novosProfessores} | turmas novas: ${novasTurmas} | lotações novas: ${lotacoesCount} | matriz_projetos_escola novas: ${mpeCount} | alocações novas: ${alocCount} | pendências novas: ${pendenciasNovas}`);
  } catch (e) {
...
async function main() {
  ...
  if (!filePath) {
    console.error('Uso: node tools/parse-planilha.js "<arquivo.xlsx>" [--import] [--force]');
    process.exit(1);
  }
  ...
  if (faltando.length) {
    console.error('Abas esperadas não encontradas neste arquivo:', faltando.join(', '));
    console.error('Abas disponíveis:', wb.worksheets.map(w => w.name).join(', '));
    process.exit(1);
  }
  console.log('Abas usadas:', { equipe: wsEquipe.name, rh: wsRH.name, grade: wsGrade.name, matriz: wsMatriz.name });
  ...
  // ---- checar soma de períodos por turma x disciplina vs matriz curricular do próprio arquivo ----
  const somaPorTurmaDisciplina = {};
  for (const a of alocacoesResolvidas) {
    // "Regente" cobre várias disciplinas simultaneamente (LP/ER/M/C/H/G) — trata à parte
    if (a.sigla === 'Regente') continue;
    const key = `${a.turma}|${a.sigla}`;
    somaPorTurmaDisciplina[key] = (somaPorTurmaDisciplina[key] || 0) + 1; // 1 professor = ok, cobertura por presença
  }
  ...
  await gerarDevolutiva(pendencias, nomeEscola, devolutivaPath);
  ...
  // ---- import ----
  try {
    await importar(pool, filePath, { rosterValido, cargoPorMatricula, grade, alocacoesResolvidas, matriz, pendencias });
  } finally {
    await pool.end();
  }
}
